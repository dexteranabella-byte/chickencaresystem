...
def create_default_admin():
    ...
    try:
        ...
        admin_user = os.environ.get("SUPER_ADMIN_USER")
        admin_email = os.environ.get("SUPER_ADMIN_EMAIL")
        admin_pass = os.environ.get("SUPER_ADMIN_PASS")

        # If any are missing, log a warning and stop.
        if not all([admin_user, admin_email, admin_pass]):
            app.logger.warning("SUPER_ADMIN variables not set. Default admin will not be created.")
            return
        ...
        with get_conn() as conn, conn.cursor() as cur:
            ...
            if not cur.fetchone():
                ...
                admin_role = "admin" # Or 'superadmin' if you have special logic for it
                
                cur.execute(
                    "INSERT INTO users (username, email, password, role) VALUES (%s, %s, %s, %s)",
                    (admin_user, admin_email, hashed_pass, admin_role)
                )
                app.logger.info(f"Default admin account created (user: {admin_user}, email: {admin_email}).")
    except Exception as e:
...
# Run startup tasks
init_tables()
create_default_admin() # Create the default admin from ENV VARS
